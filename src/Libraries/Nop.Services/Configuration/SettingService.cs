...
public partial class SettingService : ISettingService
{
    ...
    #region Utilities
        ...
        public async Task<UncDriveMapping> PictureVaultRootFolderAsync()
        {
            var pictureVaultRootFolder = await GetDatabaseConfigAsync("PictureVaultRootFolder");
            return JsonConvert.DeserializeObject<UncDriveMapping>(pictureVaultRootFolder);
        }

        public async Task<UncDriveMapping> WebAttachmentsRootFolderAsync()
        {
            var rootFolder = await GetDatabaseConfigAsync("WebAttachmentRootFolder");
            return JsonConvert.DeserializeObject<UncDriveMapping>(rootFolder);
        }

        public async Task<string> GetDatabaseConfigAsync(string key)
        {
            try
            {
                var keyParameter = new DataParameter("key", key, DataType.NVarChar);
                return (await _dataProvider.QueryAsync<string>("select Value from albina.Configuration WHERE Search = @key", keyParameter)).SingleOrDefault();
            }
            catch
            {
                return null;
